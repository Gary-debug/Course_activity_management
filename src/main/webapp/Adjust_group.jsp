...
<%
	...
	ResultSet rs=stmt.executeQuery("SELECT * FROM `studb01`.`grade` ORDER BY `ano` ASC LIMIT 1000");
 ...
 <td><a href="edit_group.jsp?sno=<%=rs.getString("sno")%> &ano=<%=rs.getString("ano") %> &tid=<%=tid%>&tpwd=<%=tpwd%>">调整分组</a></td>
 <%} %>
  </tr>
