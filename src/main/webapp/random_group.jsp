...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-body">
	...
	<td>活动:</td>
	<td><select name="ano">
    <option selected>请选择活动</option>
           <%
           javax.sql.rowset.WebRowSet rs=sqldb.DBtool.query("select distinct ano,aname from activity ");
           while(rs.next()){
...
ResultSet rs2=null;
if(cno!=null){	
	rs2=stmt.executeQuery("select * from class where cno="+cno+" ");
}
...
javax.sql.rowset.WebRowSet rs_ano_aname = sqldb.DBtool.query("select * from activity where cno="+cno+" and ano="+ano+" ");

ArrayList<Integer> num_list=new ArrayList<>();
while(rs_ano_aname.next()){
	num_list.add(rs_ano_aname.getInt("max_num"));
	k = num_list.get(0);
}
k = (int) Math.ceil(n/k);
...
int cnt=0;
for(int i=0;i<n;i++){
    gno[m.get(arr[i])-1]=((cnt++)%k)+1;
}
System.out.print("分组：");
for(int i=0;i<n;i++) {
    System.out.print(gno[i]+" ");
    sqldb.DBtool.update("update grade set gno="+gno[i]+" where ano="+ano+" and gno is NULL limit 1" );
}
%>
