...
%>
<script type="text/javascript">alert("你已在小组中！");</script>
<%}else if("2".equals(op)){%>
	<script type="text/javascript">alert("你不在该小组中！");</script>
<%}else if("3".equals(op)){ %>
<script type="text/javascript">alert("你已在该小组中！");</script>
<%}else if("4".equals(op)){ %>
<script type="text/javascript">alert("加入失败！已超出小组最大人数限制！");</script>
<%}else{}%>
<body>
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-body">
	...
	<form name="f" action="class.jsp" method="post">
		...
		<button class="layui-btn" style="margin-left:3%;margin-top:1%" onclick="javascript:document.f.action='actinfo0.jsp';document.f.submit();">进入活动</button>
		</form>
	<table border="1" width="90%" style="font-size:1em;margin-left:3%;margin-top:1%">
		<tr>
			<th width="88%">小组成员（每小组不超过<%=wrrs.getString("max_num") %>人）</th>
			<th></th>
		</tr>
		...
		<tr>
			...
			<td align="center">
			<%
			ResultSet rrsss=stmt.executeQuery("select * from grade where ano='"+ano+"' and gno='"+g.get(i)+"' and sno='"+Sno+"'");
			boolean flag=false;
			if(rrsss.next()){
				flag=true;
			}
			rrsss.close();
			if(flag){%>
			<button class="layui-btn" onclick="location='quitgrp.jsp?ano=<%=ano%>&pgno=<%=g.get(i)%>'">退出</button></td>
			<%}else{%>
			<button class="layui-btn" onclick="location='joingrp.jsp?ano=<%=ano%>&pgno=<%=g.get(i)%>'">加入</button>
			<%} %>
		</tr>
		...
		<form action="addgrp.jsp" method="post">
		<input  name="ano" value=<%=ano %> type="hidden"/>
		<button class="layui-btn" style="margin-left:3%;margin-top:1%">创建新的小组</button>
		</form>
