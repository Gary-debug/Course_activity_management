...
ResultSet rs1 = stmt1.executeQuery("select * from tea_class where cno='"+cno+"'");
if(rs1.next()){
...
<table>
<th>
<form action="AddStudent.jsp?cno=<%=cno%>&cname=<%=cname%>&tid=<%=tid%>&tpwd=<%=tpwd%>" method="post">
<button style = "margin-left:70px" type="submit">添加学生</button>
</form>
</th>
<th>
<form action="DeleteStudent.jsp?cno=<%=cno%>&cname=<%=cname%>&tid=<%=tid%>&tpwd=<%=tpwd%>" method="post">
<button style = "margin-left:70px" type="submit">删除学生</button>
</form>
</th>
</table>
	...
	<tr>
		...
		<td align="center"><%=sname%></td>
 		<td align="center">
 		<a href="DeleteStu.jsp?tid=<%=tid%>&tpwd=<%=tpwd%>&cno=<%=cno%>&cname=<%=cname%>&sno=<%=sno%>&sname=<%=sname%>"onClick="return confirm('确定删除？')"><strong>删除</strong></a>
		</td>
	</tr>
	</tbody>
    <% } %>
...
<form action="Class1.jsp?cno=<%=cno%>&cname=<%=cname%>&tid=<%=tid%>&tpwd=<%=tpwd%>" method="post">
<button style = "margin-left:570px" type="submit">完成</button>
	<% rs.close();
		stmt.close();
		conn.close();
